...
export default function EgliseDashboardPage() {
  const searchParams = useSearchParams()
  const eglise = searchParams.get('eglise') || 'Église Saint Jean Bosco'
  const { userRole } = useAuth()
  
  // Obtenir l'ID de la eglise depuis le profil utilisateur ou utiliser une valeur par défaut
  const parishId = userRole?.parishId || 'eglise-saint-jean-bosco'
  
  const [stats, setStats] = useState<EgliseStats>({
    totalFideles: 0,
    totalDonations: 0,
    totalAmount: 0,
    activeFideles: 0,
    ...
    try {
      ...
      if (donations.length > 0 || activities.length > 0 || users.length > 0) {
        toast({
          ...
          description: "Les données de la eglise ont été chargées avec succès"
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Erreur",
        description: "Impossible de charger les données du tableau de bord eglise",
        variant: "destructive"
      });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 via-white to-emerald-100">
        <div className="flex items-center gap-3">
          ...
          <span className="text-lg text-gray-600">Chargement du tableau de bord eglise...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-100 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Tableau de bord eglise</h1>
            <p className="text-gray-600 mt-1">
              {stats.egliseName} • Dernière mise à jour: {lastUpdate.toLocaleTimeString("fr-FR")}
            </p>
          </div>
          <div className="flex gap-3">
            <Button 
              onClick={loadEgliseData} 
              variant="outline" 
            ...
            <CardContent className="p-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600">Église opérationnelle</span>
                </div>
                <div className="flex items-center gap-2">
        ...
        <Card className="border-0 shadow-lg">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              <Link href={`/admineglise/prayers?eglise=${encodeURIComponent(eglise)}`}>
              ...
              <Link href={`/admineglise/donation-types?eglise=${encodeURIComponent(eglise)}`}>
              ...
              <Link href={`/admineglise/donations?eglise=${encodeURIComponent(eglise)}`}>
              ...
              <Link href={`/admineglise/news?eglise=${encodeURIComponent(eglise)}`}>
