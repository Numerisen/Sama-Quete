...
export default function AdminParoisseNewsPage() {
  ...
  useEffect(() => {
    const loadNews = async () => {
      try {
        ...
        const { NewsService } = await import("@/lib/firestore-services")
        const firestoreNews = await NewsService.getAll()
        const paroisseNews = firestoreNews.filter(n => n.paroisse === paroisse || n.parish === paroisse)
        setNews(paroisseNews)
            ...
            {paginatedNews.length === 0 && (
              <div className="text-center py-16">
                ...
                <p className="text-gray-600 mb-6">
                  {news.length === 0 
                    ? `Aucune actualité n'est enregistrée dans Firestore pour le diocèse ${paroisse}.`
