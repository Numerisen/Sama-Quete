...
function AdminParoisseUsersContent() {
  ...
  const loadUsers = async () => {
    try {
      ...
      // Filtrer par parishId si disponible, sinon par nom de paroisse
      const paroisseUsers = parishId 
        ? firestoreUsers.filter(u => u.parishId === parishId || u.paroisse === paroisse || u.parish === paroisse)
        : firestoreUsers.filter(u => u.paroisse === paroisse || u.parish === paroisse)
      const paroisseParishes = firestoreParishes.filter(p => 
        (parishId && p.id === parishId) || p.paroisse === paroisse || p.name === paroisse
      )
