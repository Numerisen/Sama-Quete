...
import { Home, Users, Newspaper, DollarSign, Church, Bell, Tag, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function AdminParoisseAdminSidebar() {
  ...
  const links = [
    ...
    { 
      href: `/adminparoisse/eglises?paroisse=${encodeURIComponent(paroisse)}`, 
      label: "Églises", 
      icon: <Church className="w-6 h-6" /> 
    },
    { 
      href: `/adminparoisse/users?paroisse=${encodeURIComponent(paroisse)}`, 
      label: "Utilisateurs", 
    ...
    { 
      href: `/adminparoisse/notifications?paroisse=${encodeURIComponent(paroisse)}`, 
      label: "Notifications", 
    ...
    <aside className={`fixed left-0 top-0 h-full bg-gradient-to-b from-blue-600 to-indigo-600 text-white shadow-lg z-40 flex flex-col transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'}`}>
      ...
      {!collapsed && (
        <div className="px-6 py-3 bg-white/10 border-b border-white/10">
          ...
          <div className="text-sm font-medium text-white truncate">{paroisse}</div>
        </div>
      )}
