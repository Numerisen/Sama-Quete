...
interface ProtectedRouteProps {
  ...
  requiredRole?: 'admin' | 'super_admin' | 'diocese' | 'paroisse' | 'church' | 'any'
}

export default function ProtectedRoute({ children, requiredRole = 'any' }: ProtectedRouteProps) {
  const { user, userRole, loading, isAdmin, isDioceseAdmin, isParishAdmin, isChurchAdmin } = useAuth()
  ...
  useEffect(() => {
    if (!loading) {
      ...
      if (requiredRole === 'church' && !isChurchAdmin) {
        router.push('/admin/dashboard')
        return
      }
    }
  }, [user, userRole, loading, isAdmin, isDioceseAdmin, isParishAdmin, isChurchAdmin, router, requiredRole])
